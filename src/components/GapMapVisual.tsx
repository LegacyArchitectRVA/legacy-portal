 ...
 * rather than referenced by file path. External image references don't
 * reliably resolve inside a WebGL texture load in some export contexts, so
 * this stays self-contained same as it always was. Downsized to 120x120
 * before encoding since they only ever render small here.
  ...
  function GapMapVisual({ scores, readiness }, ref) {
    ...
    useEffect(() => {
      ...
      scores.forEach((s, i) => {
        ...
        const glow = makeGlowSprite(color, 1.7);
        glow.position.y = 0.02;
        node.add(glow);

        const gemGeo = new THREE.IcosahedronGeometry(0.44, 1);
        ...
        if (iconSrc) {
          ...
          textureLoader.load(iconSrc, (iconTexture) => {
            ...
            iconMat.map = iconTexture;
            iconMat.opacity = 1;
            iconMat.needsUpdate = true;
            disposables.push(iconTexture);
            composer.render();
          });
          disposables.push(iconMat);
        }
