...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <div className="px-4 pt-5 pb-2 font-heading text-[11.5px] sm:text-[10px] tracking-widest text-gold-muted uppercase">
      {children}
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen bg-black flex flex-col">
      ...
      <div className="flex-1 overflow-y-visible">
        ...
        <Row
          icon={<LayoutDashboard className="w-4 h-4 text-gold-muted" />}
          label="Dashboard"
          onClick={() => go("/dashboard")}
        />
        <Row
          icon={<User className="w-4 h-4 text-gold-muted" />}
          label="Profile"
          onClick={() => go("/profile")}
        />
        <Row
          ...
          trailing={
            unreadCount && unreadCount > 0 ? (
              <span className="bg-gold-primary text-[#0f0c08] text-[11.5px] sm:text-[10px] font-bold rounded-full w-5 h-5 flex items-center justify-center">
                {unreadCount}
              </span>
            ) : undefined
          }
        ...
        <Row
          icon={<BookOpen className="w-4 h-4 text-gold-muted" />}
          label="Introduction"
        ...
        {chapters.map(ch => {
          ...
          return (
            <Row
              ...
              trailing={
                !accessible ? (
                  <span className="text-[10.3px] sm:text-[9px] bg-gold-dark/20 text-gold-muted px-1.5 py-0.5 rounded-full">
                    {ch.tier}
                  </span>
                ) : (
        ...
        {isAdmin && (
          <>
            ...
            />
            <Row
              icon={<UserCog className="w-4 h-4 text-gold-muted" />}
              label="User Access"
              onClick={() => go("/admin/users")}
            />
            <Row
              icon={<Users className="w-4 h-4 text-gold-muted" />}
              label="Prospects"
              onClick={() => go("/admin/prospects")}
            />
            <Row
              icon={<TrendingUp className="w-4 h-4 text-gold-muted" />}
              label="Blueprint Sessions"
              onClick={() => go("/admin/blueprint")}
            />
            <Row
              icon={<BookOpen className="w-4 h-4 text-gold-muted" />}
            ...
            />
            <Row
              icon={<File className="w-4 h-4 text-gold-muted" />}
              label="Document Conversion"
              onClick={() => go("/convert")}
            />
            <Row
              icon={<PaintBrush className="w-4 h-4 text-gold-muted" />}
              label="Visual Editor"
              onClick={() => go("/admin/visual-editor")}
            />
            <Row
              icon={<PlugsConnected className="w-4 h-4 text-gold-muted" />}
              label="HubSpot Settings"
              onClick={() => go("/admin/hubspot")}
            />
          </>
        )}
        ...
        />
        <Row
          icon={<Settings className="w-4 h-4 text-gold-muted" />}
          label="Settings"
          onClick={() => go("/settings")}
        />
        <Row
          icon={<LogOut className="w-4 h-4 text-gold-muted" />}
          label="Sign Out"
