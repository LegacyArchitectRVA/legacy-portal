...
function orderByCanonicalSection(pageGroups: { title: string; blocks: Block[] }[]): Block[] {
  ...
  buckets.forEach((groups) => {
    ...
    // own subpages.
    groups.sort((a, b) => {
      const ao = /overview/i.test(a.title) ? 0 : 1;
      const bo = /overview/i.test(b.title) ? 0 : 1;
      return ao - bo;
    });
    for (const g of groups) out.push(...g.blocks);
  });
  // Anything unrecognized goes at the end rather than being lost.
  for (const g of unmatched) out.push(...g.blocks);
  return out;
}
...
function isJunkPage(title: string, blockMap: any): boolean {
  ...
  // Internal SOP / process material — must never reach a client file.
  ...
  // Gather a rough plain-text length + detect AFFiNE tutorial fingerprints.
  let textLen = 0;
  let sawTutorial = false;
  try {
    for (const block of blockMap.values()) {
      const txt = block?.get?.("prop:text");
      const s = txt && typeof txt.toString === "function" ? txt.toString() : "";
      if (s) {
        textLen += s.trim().length;
        const low = s.toLowerCase();
        if (
          // AFFiNE built-in tutorial fingerprints
          low.includes("welcome to affine") ||
          low.includes("rabbit hole") ||
          low.includes("edgeless canvas") ||
          low.includes("docs.affine.pro") ||
          // Internal SOP / scaffolding that leaked into the workspace body
          low.includes("everything is now done") ||
          low.includes("internal sop for delivering") ||
          low.includes("emojioptimized sidebar") ||
          low.includes("master template -- duplicate") ||
          low.includes("client onboarding checklist")
        ) {
          sawTutorial = true;
        }
      }
    ...
    // If we can't introspect, don't filter — better to keep than wrongly drop.
    return false;
  }

  if (sawTutorial) return true;

  // Blank/near-blank "Untitled" pages: no title and essentially no body.
  if ((t === "" || t === "untitled") && textLen < 40) return true;

  return false;
}

/**
 * Removes cross-reference links that point at pages which no longer
 * exist in this workspace (dead AFFiNE anchors like "[Attorney](#attorney)"
...
function stripDeadLinks(blocks: Block[], slugByTitle: Map<string, string>): void {
  ...
  for (const b of blocks) {
    ...
    else if (b.type === "heading") b.text = clean(b.text);
...
export async function parsePdf(
  ...
  for (const line of lines) {
    ...
    // Structure recovery for flattened PDFs: a line is a section heading if
    // it MATCHES A KNOWN LIFE MANUAL SECTION TITLE, even when the PDF export
    // crushed all font sizes so ratio-based detection can't see it. This is
    // what lets the cleaner find and reorder sections in PDF/HTML input.
    const looksLikeSectionTitle =
      // "01: DIGITAL LIFE - OVERVIEW" style numbered section
      /^\d{1,2}\s*[:.]\s*[A-Z].*(overview)?/i.test(text) ||
      isKnownSectionTitle(text);
...
export function downloadBlob(blob: Blob, filename: string) {
  ...
  a.download = filename;
  a.rel = "noopener";
  // Some mobile browsers require the anchor to be in the DOM to honor a click.
  a.style.display = "none";
  document.body.appendChild(a);
  a.click();
  // Do NOT revoke immediately: on mobile the download starts asynchronously and
  // revoking the object URL right away cancels it before the file is fetched,
  // which looks like "nothing happened." Give the browser time, then clean up.
  setTimeout(() => {
