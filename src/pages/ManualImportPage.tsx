...
export default function ManualImportPage() {
  ...
  if (!isAdmin) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-16 text-center text-sm text-[#e8e6e1]/75">
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="bg-[#0a0a0a] rounded-xl border border-gold-border p-5 space-y-4">
        <div>
          <label className="text-xs text-[#e8e6e1]/75 uppercase tracking-wider font-heading">Client</label>
          <select
            value={clientUserId}
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="flex items-center gap-2 bg-gradient-to-r from-[#d9cca0] to-[#b89f6b] text-[#0a0a0a] font-heading text-sm font-semibold px-5 py-2.5 rounded-lg hover:opacity-90 transition-opacity disabled:opacity-40"
            >
          ...
          {chunks.map((c) => (
            <div
              key={c.id}
              className={`bg-[#0a0a0a] rounded-xl border p-4 space-y-3 ${c.include ? "border-gold-border" : "border-gold-border/20 opacity-50"}`}
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <label className="flex items-center gap-1.5 text-[10px] text-[#e8e6e1]/70 shrink-0">
                  <input
                    type="checkbox"
                    checked={c.include}
              ...
              <select
                ...
                className={`w-full bg-black border rounded-lg px-3 py-2 text-xs text-[#e8e6e1] focus:outline-none appearance-none cursor-pointer ${c.target ? "border-gold-border/40" : "border-amber-500/50"}`}
              >
              ...
              {c.text ? (
                <textarea
                  ...
                  className="w-full bg-black border border-gold-border/30 rounded-lg px-3 py-2 text-xs text-[#e8e6e1]/90 focus:border-gold-primary/50 focus:outline-none font-mono"
                />
              ) : null}
